import React, {useState, useReducer} from 'react';
import PropTypes from 'prop-types';
import Select from '@mui/material/Select';
import './Shared.css';
import axios from "axios";
import {Avatar, MenuItem, Typography, TextField, Checkbox, Button, Alert} from "@mui/material";
import DownhillSkiingIcon from '@mui/icons-material/DownhillSkiing';

function reducer(state, action) {
    switch (action.type) {
        case 'initial':
            return {error: null, info: null, spinner: false};
        case 'saving':
            return {error: null, info: null, spinner: true};
        case 'error':
            return {error: action.error, info: null, spinner: false};
        case 'saved':
            return {error: action.error, info: "Saved", spinner: false};
        default:
            throw new Error();
    }
}

export default function Athlete({athlete}) {
    const initialState = {status: 'initial'};
    const [state, dispatch] = useReducer(reducer, initialState);

    const saveVol = async e => {
        e.preventDefault();
        //await saveVol(volunteer, dispatch)
    }

    let peep = athlete ? athlete : {};

    let alert;
    if (state.error) {
        alert = <Alert severity="error">{state.error}</Alert>
    } else if (state.info) {
        <Alert severity="info">{state.info}</Alert>
    } else {
        alert = <span></span>
    }

    let datePicker = <span></span>;
    //datePicker = <MobileDatePicker required id='birthdate' label='Birthdate' value={volunteer.birthdate}/>

    return (
        <div>
            <Typography variant='caption' sx={{ fontSize: 24 }} color="text.secondary" gutterBottom>Athlete</Typography>
            <div className='right'>
                <Avatar><DownhillSkiingIcon/></Avatar>
            </div>

            {alert}

            <form onSubmit={saveVol}>
                <div className='form-div'>
                    <div className='form-element'><TextField fullWidth required id="first_name" label="First Name"
                                                             variant="outlined" value={peep.firstname}/></div>
                    <div className='form-element'><TextField fullWidth required id="last_name" label="Last Name"
                                                             variant="outlined" value={peep.lastname}/></div>
                    {datePicker}
                    <div className='form-element'><TextField fullWidth required id="phone_primary" label="Cell Phone"
                                                             variant="outlined" value={peep.phone_primary}/>
                    </div>
                    <div className='form-element'><TextField fullWidth id="phone_secondary" label="Other Phone"
                                                             variant="outlined" value={peep.phone_secondary || ''}/>
                    </div>
                    <div className='form-element'><TextField fullWidth required id="email" label="Email"
                                                             variant="outlined" value={peep.email}/></div>

                    <div>
                        <div className='form-element'><TextField fullWidth id="address" label="Address"
                                                                 variant="outlined" value={peep.address || ''}/></div>
                        <div className='form-element'><TextField fullWidth id="city" label="City" variant="outlined"
                                                                 value={peep.city || ''}/></div>
                        <div className='form-element'><TextField fullWidth id="state" label="State" variant="outlined"
                                                                 value={peep.state || ''}/></div>
                        <div className='form-element'><TextField fullWidth id="zip" label="Zip" variant="outlined"
                                                                 value={peep.zip || ''}/></div>
                    </div>

                    <div className="section">
                        <b>Availability</b><br/>
                        Tuesday 6-8p<Checkbox id="tuesday" label="Tuesday 6-8p" value={peep.tuesday}>Tuesday
                        6-8p</Checkbox>
                        Wednesday 6-8p<Checkbox id="wednesday" label="Wednesday 6-8p" value={peep.wednesday}>Wednesday
                        6-8p</Checkbox>
                        Thursday 6-8p<Checkbox id="thursday" label="Thursday 6-8p" value={peep.thursday}>Thursday
                        6-8p</Checkbox>
                        Sunday 1-3p<Checkbox id="sunday1" label="Sunday 1-3p" value={peep.sunday1}>Sunday
                        1-3p</Checkbox>
                        Sunday 2-4p<Checkbox id="sunday2" label="Sunday 2-4p" value={peep.sunday2}>Sunday
                        2-4p</Checkbox>
                        Sunday 3-5p<Checkbox id="sunday3" label="Sunday 3-5p" value={peep.sunday3}>Sunday
                        3-5p</Checkbox>
                    </div>
                    <div>
                        <Button variant="contained" type="submit">Save</Button>
                    </div>
                </div>
            </form>
        </div>
    );

}

Athlete.propTypes = {
    athlete: PropTypes.object.isRequired
}